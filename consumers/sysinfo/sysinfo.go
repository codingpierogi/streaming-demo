...
import (
	...
	"github.com/codingpierogi/streaming-demo/consumers/sysinfo/config"
	"github.com/codingpierogi/streaming-demo/consumers/sysinfo/util"
...
func main() {
	...
	log.Println("Starting SysInfo Consumer...")

	config, err := config.LoadConfig("consumers/sysinfo/")
...
func consumeSysInfoMessage(e *kafka.Message) {
	sim, err := util.DeserializeMessage(e.Value)

	if err != nil {
		log.Fatalf("Fatal message deserialization error: %v\n", err)
	}

	log.Printf("Consumed message [topic=%s] [time=%s] [message=%+v]\n", *e.TopicPartition.Topic, e.Timestamp, sim)
}
